...
export class PeopleTableComponent implements OnInit {
  ...
  populateTable(facets = {}) {
    ...
    if (
      facets['expertiseTopics'] !== undefined &&
      facets['expertiseTopics'].length
    ) {
      ...
      this.queryService.getSubTopics(expertiseTopics).subscribe({
        ...
        error: (response) => {
          console.error(
            'There was an error while retrieving subtopics',
            response
          )
          this.totalSize = 0
          this.resultsCountEvent.emit(this.totalSize)
    ...
    this.queryService
      ...
      .then((response: any) => {
        ...
        response.results['results']['bindings'].forEach((result) => {
          ...
          expertise = result['expertise']['value']
            .split(', ')
            .map(function (x, i) {
          ...
          this.people.push({
            ...
            place: result['affiliationQuantName']
              ? result['affiliationQuantName']['value']
              : '',
            place_uri: result['affiliationLoc']
              ? result['affiliationLoc']['value']
              : '',
        ...
        this.queryService
          .query(
            `select (count(*) as ?count) { ` + response.query + ` } LIMIT 200`
          )
          .then((res) => {
