...
export class HazardsTableComponent implements OnInit {
  ...
  getResultsSize() {
    this.queryService
      .getHazardCount(this.pageSize * 10, this.currentPage)
      .subscribe({
        next: (response) => {
        ...
        error: (response) => {
          console.error(
            'There was an error while retrieving the number of results',
            response
          )
        },
      })
  ...
  populateTable(facets = {}) {
    ...
    // hazard URI's.
    this.queryService
      .getHazardSearchResults(facets, this.pageSize, this.offset)
      .then((results: any) => {
        if (results === false) {
          this.totalSize = 0
          this.resultsCountEvent.emit(this.totalSize)
        ...
        this.queryService
          .getHazardProperties(hazardUris, this.pageSize)
          .subscribe({
            next: (response) => {
              if (response === false) {
                this.totalSize = 0
              ...
              hazard_properties.forEach((row) => {
                ...
                record.place =
                  typeof row.place === 'undefined' ? '' : row.place.value
                record.placeName =
                  typeof row.placeQuantName === 'undefined'
                    ? ''
                    : row.placeQuantName.value
                record.placeWkt =
                  typeof row.placeWkt === 'undefined' ? '' : row.placeWkt.value
                record.startDate =
                  typeof row.time === 'undefined' ? '' : row.time.value
                record.startDateName =
                  typeof row.startTimeLabel === 'undefined'
                    ? ''
                    : row.startTimeLabel.value
                record.endDate =
                  typeof row.time === 'undefined' ? '' : row.time.value
                record.endDateName =
                  typeof row.endTimeLabel === 'undefined'
                    ? ''
                    : row.endTimeLabel.value
                record.wkt = typeof row.wkt === 'undefined' ? '' : row.wkt.value
                let resTypes = row['typeLabel']['value']
                  .split(',')
                  .filter(function (resType) {
                    return resType !== 'NIFCWildfire'
                  })
